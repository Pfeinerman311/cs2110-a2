...
/* NetIds: pjf73. Time spent: 1 hours, 45 minutes. */
...
public class A2 {
	...
	public static double timeSpent= 1.45;
	...
	public static boolean isMidSame(String s) {
		...
		int n= s.length();
		int i= n / 2;
		if (n == 1 || n % 2 == 1 && s.charAt(i - 1) == s.charAt(i + 1)) { return true; }
		return false;
	}
	...
	public static boolean isCatenated(String s, int k) {
		...
		if (k > s.length() || s.length() % k != 0) { return false; }
		String cat= s.substring(0, k);
		for (int i= k; i < s.length(); i= i + k) {
			if (!cat.equals(s.substring(i, i + k))) { return false; }
		}
		return true;
	}
	...
	public static String findShort(String s) {
		...
		for (int i= 1; i <= s.length(); i++ ) {
			if (isCatenated(s, i)) { return s.substring(0, i); }
		}
		return s;
	}
}
